...
int main(int argc, char *argv[])
{
	...
	char * forUDPCheckSum = malloc(sizeof(struct UDP_PSD_Header) + sizeof(struct udphdr)+sizeof(DNS)+1);
	memset(forUDPCheckSum, 0, sizeof(struct UDP_PSD_Header) + sizeof(struct udphdr) + sizeof(DNS) + 1);
	struct UDP_PSD_Header * udp_psd_Header = (struct UDP_PSD_Header *)forUDPCheckSum;
	...
	memcpy(forUDPCheckSum + sizeof(struct UDP_PSD_Header), udp, sizeof(struct udphdr));
	memcpy(forUDPCheckSum + sizeof(struct UDP_PSD_Header) + sizeof(struct udphdr), DNS, sizeof(DNS));

	//ip->check = csum((unsigned short *)ip, sizeof(iphdr)/2);//可以不用算
	//计算UDP的校验和，因为报文长度可能为单数，所以计算的时候要补0
	udp->check = csum((unsigned short *)forUDPCheckSum,(sizeof(struct udphdr)+sizeof(struct UDP_PSD_Header)+sizeof(DNS)+1)/2);
